...
import { getConversions, getRates } from "@/lib/pricing";
...
export function PaymentConversionTable({ priceArs }: { priceArs: number }) {
  ...
  const hasUsdRate = rates.usdArs > 0;
  const usdRateText = hasUsdRate
    ? `* Cotización tomada a 1 USD = $${formatARS(rates.usdArs)} ARS`
    : `* Falta configurar NEXT_PUBLIC_USD_ARS en Vercel`;
    ...
    <div className="bg-zinc-900/60 backdrop-blur-md p-6 rounded-2xl border border-white/10 my-6 shadow-xl relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-4 text-sm relative z-10">
        ...
        <div className="p-4 bg-black/50 border border-zinc-800 rounded-xl hover:border-zinc-600 transition-colors">
          ...
          <p className="text-blue-400 font-black text-xl tracking-tight">
            U$D {formatUSD(conversions.usd)}
          </p>
          {/* Tip extra: si tenés USDT distinto al USD */}
          {rates.usdtArs > 0 && rates.usdtArs !== rates.usdArs && (
            <p className="text-[10px] text-zinc-500 font-bold mt-1 uppercase tracking-widest">
              USDT: U$D {formatUSD(conversions.usdt)}
            </p>
          )}
        </div>
