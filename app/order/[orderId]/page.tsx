    ...
    <div className="min-h-screen bg-black text-white p-4 md:p-10 font-sans flex justify-center">
      <div className="max-w-3xl w-full space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-black italic uppercase tracking-tighter">¡Orden Generada!</h1>
        ...
        <div className="bg-[#111] border border-zinc-800 rounded-3xl p-6 shadow-2xl">
          <div className="flex justify-between items-end border-b border-zinc-800 pb-5 mb-5">
            <div>
              <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Total a Pagar</p>
            ...
            <div className="text-right">
              ...
              <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${order.status === 'verifying' ? 'bg-yellow-500/10 text-yellow-500' : 'bg-zinc-800 text-white'}`}>
          ...
          <div className="space-y-6">
            <h3 className="text-center text-xs font-bold uppercase tracking-widest text-zinc-500">
            ...
            {order.payment_method === 'transfer_ars' && (
              <div className="bg-zinc-950 p-5 rounded-xl border border-zinc-800 space-y-4">
                <p className="text-emerald-500 text-xs font-black uppercase text-center mb-2">{PAYMENT_CONFIG.brubank.title}</p>
            ...
            {order.payment_method === 'crypto' && (
              <div className="space-y-3">
                <div onClick={() => copyToClipboard(PAYMENT_CONFIG.crypto.usdt)} className="bg-zinc-950 p-4 rounded-xl border border-zinc-800 cursor-pointer hover:border-emerald-500/50 transition-all group">
                  <div className="flex justify-between mb-1">
                    <span className="text-emerald-500 text-[10px] font-black uppercase">USDT (TRC20)</span>
                    <span className="text-[10px] text-zinc-600 group-hover:text-white">COPIAR</span>
                  </div>
                  <p className="font-mono text-xs text-zinc-300 break-all group-hover:text-white">{PAYMENT_CONFIG.crypto.usdt}</p>
                </div>
                <div onClick={() => copyToClipboard(PAYMENT_CONFIG.crypto.usdc)} className="bg-zinc-950 p-4 rounded-xl border border-zinc-800 cursor-pointer hover:border-blue-500/50 transition-all group">
                  <div className="flex justify-between mb-1">
                    <span className="text-blue-500 text-[10px] font-black uppercase">USDC (Polygon)</span>
                    <span className="text-[10px] text-zinc-600 group-hover:text-white">COPIAR</span>
                  </div>
                  <p className="font-mono text-xs text-zinc-300 break-all group-hover:text-white">{PAYMENT_CONFIG.crypto.usdc}</p>
                </div>
              </div>
            )}

            {/* ─── CASO 3: DÓLAR INTERNACIONAL ─── */}
            {(order.payment_method === 'international_usd' || order.payment_method === 'usd') && (
              <div className="space-y-4">
                <div className="bg-zinc-950 p-5 rounded-xl border border-zinc-800">
                  <p className="text-emerald-500 text-xs font-black uppercase mb-3">{PAYMENT_CONFIG.buenbit_local.title}</p>
            ...
            {(order.payment_method === 'mercado_pago' || order.payment_method === 'mercadopago') && (
               <div className="text-center space-y-4">
                 ...
                 {/* Acá deberíamos mostrar el link si lo guardamos, o redirigir a WP como fallback */}
                 <a href={`https://wa.me/5491123021760?text=Link MP orden ${order.order_id}`} className="block w-full bg-[#009EE3] hover:bg-[#008ED6] text-white font-black py-4 rounded-xl uppercase">Pedir Link Manualmente</a>
               </div>
            )}
          </div>
        </div>
        ...
        {order.payment_method !== 'mercado_pago' && order.payment_method !== 'mercadopago' && order.status !== 'verifying' && (
          <div className="bg-[#111] border border-zinc-800 rounded-3xl p-6 shadow-2xl text-center">
            <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-4">Ya realicé el pago</h3>
            <label className="block w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-4 rounded-xl uppercase tracking-widest transition-all cursor-pointer shadow-lg shadow-emerald-500/20 active:scale-[0.98]">
        ...
        <button onClick={() => window.location.href = '/'} className="w-full text-zinc-600 hover:text-white text-xs font-bold uppercase transition-colors py-4">Volver al Inicio</button>
      </div>
    </div>
  );
}