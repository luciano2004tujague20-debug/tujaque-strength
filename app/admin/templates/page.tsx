...
export default function AdminTemplates() {
  ...
  if (loading) return <div className="p-20 text-center text-emerald-500 font-black uppercase italic animate-pulse">Abriendo el archivo maestro...</div>;
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 px-4 animate-fade-in">
      ...
      <section className="bg-[#0c0c0e] border border-white/10 rounded-[2.5rem] p-10 mb-16 shadow-2xl">
        <h2 className="text-xs font-black text-emerald-500 uppercase tracking-[0.4em] mb-8 italic">Crear Nuevo Bloque Maestro</h2>
        <div className="space-y-6">
          <input 
            ...
            className="w-full bg-black/40 border border-white/5 rounded-2xl px-6 py-4 text-xs font-black uppercase tracking-widest text-white outline-none focus:border-emerald-500/50 transition-all"
          />
          <textarea 
          ...
          <button 
            ...
            className="w-full bg-emerald-500 text-black font-black uppercase text-xs py-4 rounded-2xl hover:bg-emerald-400 hover:shadow-[0_0_25px_rgba(16,185,129,0.3)] transition-all active:scale-[0.98]"
          >
        ...
        {templates.map((t) => (
          <article key={t.id} className="bg-[#111113] border border-white/5 rounded-[2rem] p-8 flex flex-col group hover:border-emerald-500/30 transition-all">
            <div className="flex justify-between items-start mb-6">
              <h3 className="text-lg font-black uppercase italic text-white tracking-tight">{t.title}</h3>
              <button 
                onClick={() => deleteTemplate(t.id)}
                className="opacity-0 group-hover:opacity-100 text-[8px] font-black text-red-500/50 hover:text-red-500 uppercase transition-all"
              >
                Eliminar
              </button>
            </div>
            ...
            <button 
              ...
              className={`w-full py-4 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all border ${
                copyStatus === t.id 
      ...
      {templates.length === 0 && (
        <div className="py-20 text-center border-2 border-dashed border-white/5 rounded-[3rem]">
          <p className="text-zinc-700 font-black uppercase italic text-sm tracking-[0.2em]">Tu librería de plantillas está vacía</p>
        </div>
      )}
    </div>
  );
