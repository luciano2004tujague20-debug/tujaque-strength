...
export default async function TestPdfPage() {
  ...
  // 1) Buscar el entitlement por código
  const { data: entitlement, error: entitlementError } = await supabase
    .from("commerce_entitlements")
    .select("id, code")
    .eq("code", "TEST_PDF_ACCESS")
    .maybeSingle();
  ...
  if (!entitlement) {
    return (
      <main className="min-h-screen bg-black text-white p-10">
        ...
        <p>No existe el código <strong>TEST_PDF_ACCESS</strong> en commerce_entitlements.</p>
      </main>
    );
  }

  // 2) Ver si el usuario tiene ese acceso activo
  ...
  if (!hasAccess) {
    return (
      <main className="min-h-screen bg-[#050505] text-white p-8 md:p-12">
        <div className="max-w-3xl mx-auto border border-zinc-800 bg-zinc-900/50 rounded-[2rem] p-8 md:p-10">
          ...
          <p className="text-zinc-400 text-sm md:text-base leading-relaxed">
            Tu usuario todavía no tiene el entitlement{" "}
            <span className="text-emerald-400 font-bold">TEST_PDF_ACCESS</span>.
          </p>

          <div className="mt-8 bg-black/40 border border-zinc-800 rounded-2xl p-5">
            <p className="text-xs text-zinc-400">
              Cuando una compra válida se procesa correctamente, tu sistema crea un registro en{" "}
              <code className="text-emerald-400">commerce_user_entitlements</code>.
            </p>
          </div>
        </div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white p-8 md:p-12">
      <div className="max-w-4xl mx-auto border border-emerald-500/20 bg-zinc-900/50 rounded-[2rem] p-8 md:p-10">
        <p className="text-[10px] uppercase tracking-[0.3em] text-emerald-500 font-black mb-3">
          Acceso desbloqueado
        </p>

        <h1 className="text-3xl md:text-5xl font-black italic tracking-tighter mb-4">
          TEST PDF ACCESS ACTIVO
        </h1>

        <p className="text-zinc-300 text-sm md:text-base leading-relaxed">
          Perfecto. Tu sistema leyó el entitlement del usuario y desbloqueó esta página.
        </p>
