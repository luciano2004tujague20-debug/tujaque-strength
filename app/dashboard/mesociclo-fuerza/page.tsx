...
export default async function MesocicloFuerzaPage() {
  ...
  const { data: entitlement, error: entitlementError } = await supabase
    ...
    .select("id, code, name")
    .eq("code", "STATIC_STRENGTH_MESO_ACCESS")
    .maybeSingle();
  ...
  if (entitlementError) {
    return (
      <main className="min-h-screen bg-black text-white p-10">
        ...
        <p>No se pudo consultar el entitlement de fuerza.</p>
  ...
  if (!entitlement) {
    return (
      <main className="min-h-screen bg-black text-white p-10">
        ...
        <p>No existe el código <strong>STATIC_STRENGTH_MESO_ACCESS</strong>.</p>
      </main>
    );
  }
      ...
      <main className="min-h-screen bg-[#050505] text-white p-8 md:p-12">
        <div className="max-w-4xl mx-auto border border-zinc-800 bg-zinc-900/50 rounded-[2rem] p-8 md:p-10">
          ...
          <h1 className="text-3xl md:text-5xl font-black italic tracking-tighter mb-4">
            Mesociclo Fuerza no habilitado
          </h1>

          <p className="text-zinc-400 text-sm md:text-base leading-relaxed">
            Tu usuario todavía no tiene acceso activo al producto{" "}
            <span className="text-emerald-400 font-bold">Mesociclo Fuerza 4 Semanas</span>.
          </p>
        </div>
      </main>
    );
